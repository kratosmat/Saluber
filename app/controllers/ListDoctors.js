...
$.listDoctors.addEventListener("click", function(e) {	
	...
	/*
	var windowVisualizzaPrenotazione = Widget.createController("windowVisualizzaPrenotazione", {
		booking : e.row.booking,
		parkingArea: e.row.area
	});
	windowVisualizzaPrenotazione.on('update', function() {
		myRefresh();
	});
	windowVisualizzaPrenotazione.getView().open();
	*/
});
